...
// 	case "last_week_on_progress":
// 		// Apa yang sedang dikerjakan dalam 1 minggu terakhir
// 		db = db.Where("tasks.status = 'on_progress' AND tasks.updated_at BETWEEN ? AND ?", oneWeekAgo, now)
// 	case "last_week_done":
...
// 	case "last_day_on_progress":
// 		// Apa yang sedang dikerjakan dalam 1 hari terakhir
// 		db = db.Where("tasks.status = 'on_progress' AND tasks.updated_at BETWEEN ? AND ?", oneDayAgo, now)
// 	case "last_day_done":
...
func (r *taskRepository) GetTasksInProgressSince(projectID uint, since time.Time) ([]models.Task, error) {
	...
	err := db.Where("tasks.status = 'on_progress' AND tasks.updated_at >= ?", since).Find(&tasks).Error
	return tasks, err
}
